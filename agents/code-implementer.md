...
- **Assessed test design card and report requirements** — preserve every fixed heading, package-qualified selected identity, native report format, and process-exit capture requirement

For graph nodes your deliverables change shape: the coordinator holds the claim and runs every `sdd graph sync` itself. You invoke repository-owned test tooling and hand back the **untouched native RED/GREEN reports, captured process exit statuses when required, assessment findings, and workspace commit** (one clean commit of the complete slice inside the workspace). SDD validates evidence but does not execute tests. Everything else on this page — premise checks, spec fidelity, comment policy, evidence-not-assertion — applies unchanged.
...
- Check `sdd graph sync --help` and the repository's documented test-tool help. Run the exact selected identities and preserve untouched native output. If any selected test declares `package`, emit Go test JSON for the strict package-qualified parser; capture the process exit status for optional `--report-exit`. For a qualifying failure, report whether it is baseline RED or an isolated sensitivity RED and name the injected fault. Never invent or reconstruct runner output or exit status. Preserve the real GREEN report while the tested slice is committed—do not request a duplicate run solely for the commit. Keep validator facts separate from assessment judgment; distinguish absent-behavior RED, valid sensitivity RED, and deliberate compiler-rejection harness tests from accidental setup/build/import/discovery/capture failure.
...
- **git**: stage your changes and make exactly **one clean, non-merge commit per task**, message `[<Plan>/<Phase>] Task <X.Y>: <title>`; never push. The commit contains the complete task and its tests — the whole feature slice and nothing else. No planning-artifact or evidence bookkeeping goes in it (the coordinator records lifecycle once at phase close under `shared/autonomy.md` § SCM boundary cadence), and no second task's work rides along. If the task cannot land as one complete, buildable, test-passing commit, that is a plan-structure problem — STOP and report it rather than committing a half-wired state.
...
- **Verification evidence**: the exact command(s) you ran, **their working directory and exit status**, and their **pasted output** (summary tail, or the full failing section) — never a bare assertion that criteria are satisfied. The coordinator rejects evidence-free success reports and needs the exact commands to populate the task's `### Completion Evidence` section.
- **Commit hash** (git — the **full 40-hex** hash) / changelist number (perforce) / "no VCS" — plus the file list either way
