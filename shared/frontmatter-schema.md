...
- **Changing a numbered element is a reconciliation event**: after editing it, grep the other artifacts for its id and update or flag every citing site. A plan decision is never edited — a change of mind is a fresh entry with `supersedes`, per `shared/decision-log.md`. `/validate` audits for unnumbered elements and dangling id citations.

## Graph Node Roles and Amendment Fields

A node's observation (`verification`, tool-owned) records a monotonic sequence, result, `contract_rev`, isolation, report identity, and available VCS provenance. Node state derives from graph structure, observation sequence, contract revision, isolation, and red-before-green. A passing node remains GREEN until a direct dependency is deliberately re-verified at a higher sequence or its contract revision advances. Artifacts, inputs, and citations remain review-visible declarations; their bytes are not observation fields or freshness keys.

A plan's compiled graph node carries a `role` field (proposal input): `implementation` (default when absent), `shared-mechanism`, `review`, or `integration-acceptance`. A `review`-role node's `gate.type` must be `review`; a `review`-gate node must have `role: review`. `contract_rev` (an integer, starting at 1) and `origin` (present only on a node added by an `extend` amendment: `{ review: <artifact path>, finding: <id> }`) are tool-owned — refused in proposal payloads, like `claim` and `verification`. A review observation binds the reviewed scope, each reviewed node's contract revision, and its observation sequence; scope changes, revision advances, and newer reviewed observations require deliberate re-review.
