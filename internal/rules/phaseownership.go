	...
	Register(&Rule{
		...
		CheckRoot: func(r *Root, emit func(Diagnostic)) {
			...
			for _, phase := range r.Artifacts {
				...
				// Rendered graph views are projections owned by the
				// committed graph, regenerated by compile and never listed
				// in the README phases[] array v1 owns (Plans/SddGraph 5.6).
				if IsGeneratedView(phase.Source) {
					continue
				}
		...
		Good: []Example{{Name: "listed-once", Files: map[string]string{
			...
			"Plans/Sample/01-One.md": phaseDoc("Sample", "1", "One", "planned"),
		}}, {Name: "generated-view-exempt", Files: map[string]string{
			// A rendered graph view is a projection owned by the committed
			// graph, never listed in the README phases[] array v1 owns
			// (Plans/SddGraph 5.6): unlisted, but exempt by its marker.
			"Plans/Sample/README.md": planWithPhase(map[string]string{
				"id": "1", "title": "One", "status": "planned", "doc": "01-One.md",
			}),
			"Plans/Sample/01-One.md": phaseDoc("Sample", "1", "One", "planned"),
			"Plans/Sample/02-view.md": phaseDoc("Sample", "2", "view", "planned") +
				"\n" + GeneratedViewMarkerPrefix + "Sample-Graph.json. Regenerate with `sdd compile --plan Sample`. Edits here are overwritten. -->\n",
		}}},
	})
}
