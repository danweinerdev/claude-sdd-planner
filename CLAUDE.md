...
│   ├── decision-log/             #   ad-hoc plan-decision capture outside lifecycle skills
│   ├── sdd-cli/                  #   how to drive the sdd binary — task→command map + write-path discipline
│   └── test-{design,generate,assess}/ # composed test-quality instruction skills
...
│   ├── decision-log.md           #   plan decisions — per-plan JSON file, entry schema, write protocol
...
├── internal/                     # Binary internals (rules, decisions, compile, hook, provision, schema, vcs, portable)
...
│   └── Plans/<Name>/<Name>-Decisions.json  #  per-plan decisions — pd-<hex> entries are standing constraints
...
Not user-invocable (`disable-model-invocation: true`); the model auto-loads them by description. `<lang>-specifications` (7 languages) supplies structural-verification tools when planning/implementing/reviewing that language, coordinated by `shared/language-verification.md`; `test-design`, `test-generate`, and `test-assess` compose prospective test selection, authoring, and semantic result interpretation without changing graph authority; `decision-log` carries the three-step write protocol for ad-hoc plan decisions; `sdd-cli` carries the binary's task→command map and write-path discipline. In the portable trees the language skills flatten to `shared/language-specs/<lang>.md`; the test-quality standard is generated as `shared/test-design.md`; the other model-only references ship as skills. Restricted agents read skill bodies as plain files when they need them.
...
`/setup` verifies it (floor: `minSddVersion` in `plugin.json` — advanced deliberately via `bump-version.py set-floor`, never by `make bump-*`), copies it to `${CLAUDE_PLUGIN_ROOT}/bin/` for the hooks, and stops with the exact `go install` command when missing or too old. Key subcommands: `validate`, `apply`, `section set`, `evidence add`, `task|phase|plan complete`, `plan approve|activate`, `spec|design submit|approve|implement|supersede`, `decide add|list|current|lookup|render`, `review scaffold|evidence set|resolve`, `template` (incl. `graph-proposal`), `hook`, `provision`, `plugin sync|check|status`, `doctor` — plus the graph family: `compile`, `next --claim`, and `graph init|propose|assemble|convert|hazards|sync|reverify|review|amend|remap-revisions|release|split|set-tests|set-inputs|repair-red|set-artifacts|gc|retire|status|show|path|risk|shape|export|audit`.

SDD owns graph requirements and evidence validation, not test execution. Repository tooling runs declared tests and supplies untouched native output to `graph sync --report` (optionally `--report-exit`). Tests declaring `package` use the strict package-qualified Go JSON parser. Failing observations are classified with `--red-kind baseline|sensitivity` and sensitivity names `--fault`. Duplicate report bytes on one node are a no-op. There is no production `sdd test`.
...
- **Run `make test` after touching templates, the schema, or `internal/rules/`** — it runs the Go suite, the frozen regression corpus, the template gate, and the portable drift/leak gates. It also runs `go vet ./...` through the `vet` target, guarded by `TestMakeTestRunsVet`. It also depends on `make test-race`, which re-runs the shared-state packages (`internal/rules`, `internal/procexec`, `internal/vcs`, `internal/graph/{sync,ops,provider}`, `tools/regression`) under `-race` so a removed memo or containment lock fails the standard gate; `TestMakeTestRunsRaceDetector` in `tools/testgate/` refuses a `test:` recipe that drops it
- **`make test-pure` is a supplementary fast selection, never a replacement for `make test`** — it runs only the rules package's `TestPure*` cases (pure transformations: no repository, no SCM binary, no subprocess), so it is what to run while iterating on rule logic; the real-SCM boundaries and every registered Good/Bad example are covered only by the full gate. `TestPureSelectionInventory` keeps the selection non-vacuous and `TestSCMBoundaryInventory` fails when a mapped real-SCM assertion is renamed or deleted
