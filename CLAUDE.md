...
`/setup` verifies it (floor: `minSddVersion` in `plugin.json` — advanced deliberately via `bump-version.py set-floor`, never by `make bump-*`), copies it to `${CLAUDE_PLUGIN_ROOT}/bin/` for the hooks, and stops with the exact `go install` command when missing or too old. Key subcommands: `validate`, `apply`, `section set`, `evidence add`, `task|phase|plan complete`, `plan approve|activate`, `spec|design submit|approve|implement|supersede`, `decide add|list|current|lookup|render`, `review scaffold|evidence set|resolve`, `template` (incl. `graph-proposal` and `evidence-metadata`), `hook`, `provision`, `plugin sync|check|status`, `doctor` — plus the graph family: `compile`, `next --claim`, and `graph init|propose|assemble|convert|hazards|evidence-context|evidence-contract|sync|reverify|review|amend|acknowledge|remap-revisions|release|split|set-tests|set-inputs|set-artifacts|rehash|repair-intent|gc|retire|status|show|path|risk|shape|export|audit`.

SDD owns graph requirements and evidence validation, not test execution. A `reported-v1` gate declares package-qualified tests and `{format, runner, environment_keys, test_support_inputs, test_support_artifacts}`. Repository tooling captures before/after `graph evidence-context`, runs the tests, and produces native output plus strict metadata; `graph sync --report ... --metadata ...` validates and admits it. Context export executes/probes nothing and issues no receipt. There is no production `sdd test`, hidden execution flag, or new attempt admission. Historical `observed-v1` remains readable but needs explicit amendment and fresh evidence for active use; legacy imports retain their actual semantics.
