...
sets them itself and refuses a payload carrying a conflicting value.`,
	...
	c.Flags().BoolVar(&o.Schema, "schema", false, "emit the JSON Schema instead of the skeleton (graph-proposal or evidence-metadata)")
...
every 'complete' transition checks.`,
	...
	f.StringVar(&o.FinalReview, "final-review", "", "path to the phase's final aligned review artifact (normalized planning-root-relative; pair with --frozen)")
	f.StringVar(&o.Frozen, "frozen", "", "frozen identity `<base>..<endpoint>` appended to the final-review line (SDD166)")
