...
- **Design Decisions**: Each a top-level bold bullet `- **DD-N**: Title` with context, options, decision, rationale on its indented continuation lines — the bold-bullet form is the declaration `sdd apply` collects, allocates, and retires. Heading-form decisions (`### DD-N — Title`) in existing documents are recognized as declarations too — matching how `sdd validate` resolves them — but only bullet items get automatic numbering, so author new decisions as bullets. The ids are stable and append-only (`shared/frontmatter-schema.md` § Stable Identifiers): plans and phases cite them, and `sdd validate` resolves each citation against the related design (SDD122). Cite another component's decision in qualified form — `ComponentName:DD-3`.
