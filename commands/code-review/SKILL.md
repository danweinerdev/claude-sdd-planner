...
**Graph review nodes.** When the reviewed target is a graph plan's `review`-role node (the `/implement` walk invoked this review), every finding that stays `open` must be classified for the amendment path: `action: revise` with `nodes: [...]` and a `revise:` block naming the changed `contract`/`gate`/`justifies`/`inputs`, or `action: extend` with a proposal-shaped `node:` fragment whose `deps` include a reviewed node. The verdict then follows the findings: `Aligned` only when every finding is terminal (`fixed`, `deferred`, `rejected`, `answered`); `Amend` when any classified open finding remains. `sdd review resolve` freezes either shape and refuses the mixed ones; `sdd graph review` greens the node from `Aligned` and prints the amendment preview from `Amend` (`shared/review-artifacts.md` § Findings against a graph plan's review node).
...
If the user asks to address findings, follow `shared/review-artifacts.md`. Before treating a decision as governing, run `sdd decide current --plan <Name>` for the plan's standing decisions. Never pass decision context to any intent-isolated lane. A resolution that binds work beyond this artifact follows `shared/decision-log.md`: show the exact statement, then `sdd decide add --plan <Name> --statement "..."` once approved. Every disposition gets a Resolution Log entry.
