...
If the user asks to address findings, follow `shared/review-artifacts.md`. Before treating a decision as governing, inspect `decisionLog`: explicit `fork`/`detached` uses capability admission and `sdd decide effective --json`; no selector uses `sdd decide list --status accepted --json` and conventional reads. Never pass decision context to any intent-isolated lane. Design decisions follow `shared/decision-log.md`; unsupported fork writes refuse. Every disposition gets a Resolution Log entry.
