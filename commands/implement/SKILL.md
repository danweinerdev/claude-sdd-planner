...
- Run `sdd validate --scope Plans/<PlanName> --format json` and resolve any diagnostics on the tasks just completed.
...
- **Run the phase gate before flipping status.** Phase completion requires, per `shared/completion-evidence.md` and `shared/review-artifacts.md` § Phase-completion review gate:
  ...
  5. `sdd validate --scope Plans/<PlanName> --format json` passing for the phase
