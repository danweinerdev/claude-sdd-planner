...
3. **Red — design, generate, and prove the tests can fail.** From the already resolved active plugin root, read the planning design card; if missing or stale, compose `skills/sdd-test-design/SKILL.md` in place from cited intent and current code. Challenge it with `skills/sdd-test-assess/SKILL.md` (fresh non-inheriting context when useful and available; labeled same-context assessment otherwise), then generate all named tests with `skills/sdd-test-generate/SKILL.md`. Never resolve these resources from the claimed workspace. Changed identities or discovered engineering work return to proposal/amendment rather than being silently substituted. Select the node's committed evidence mode. For `observed-v1`, first run `sdd test run --help`, `sdd test check --help`, and `sdd graph sync --help`; if help lacks the required flags, stop with a binary-capability mismatch without downgrading or claiming an installed version. Baseline RED uses `sdd test run --plan <Name> --node <id> --by <identity> --phase red --red-kind baseline`; justified sensitivity RED uses the same command with `--red-kind sensitivity --fault "<named injected fault>"`. Then run `sdd test check --plan <Name> --node <id> --attempt <attempt-id> --expect red`, assess raw output and checker facts, and only when ready have the coordinator run `sdd graph sync --plan <Name> --node <id> --by <identity> --attempt <attempt-id>`. Baseline absent-behavior failure, valid sensitivity RED, and deliberate compiler-rejection harness tests are eligible; accidental build/import/setup failure and rejected diagnostics are not. Legacy gates keep the assessed targeted `--report` flow and never claim observed provenance.
4. **Green — implement, commit, sync the pass.** Implement the smallest contract-complete change without weakening assertions, hardcoding cases, or expanding node scope. Assess real output; setup/capture failure is not GREEN. For `observed-v1`, run `sdd test run --plan <Name> --node <id> --by <identity> --phase green` against final bytes, commit those same bytes without changes, run `sdd test check --plan <Name> --node <id> --attempt <attempt-id> --expect green`, then have the coordinator run `sdd graph sync --plan <Name> --node <id> --by <identity> --attempt <attempt-id>`; do not duplicate the run merely to attach a commit. The revision anchor names the tested bytes and sync requires a clean worktree. Legacy gates commit the complete tested slice and continue with `--report`. A clean pass records the observation, clears the claim, and releases the workspace; a shared-dirty legacy pass is provisional and STALE. There is no assert path.
...
Node work may be dispatched through collaboration when available. When the runtime provides a task name or description field, set it to exactly `implement_task` unchanged. Each dispatch supplies the **claim payload verbatim**, assessed design card, selected evidence mode, and red-first rule. The dispatched work composes test generation, implementation, and assessment **inside the claimed workspace**; deliverables include generated-test findings, raw execution/report material, and the workspace commit. The coordinator performs capability checks and runs every `sync` itself — graph writes and admission stay with the lease holder. Do not request an agent, worker type, provider, or model; when collaboration is unavailable, perform the same sequence yourself. Reject asserted outcomes without execution evidence.
