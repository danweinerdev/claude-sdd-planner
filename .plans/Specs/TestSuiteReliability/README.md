...
- [ ] **AC-12**: Implementation uses a binary-compiled plan graph citing this spec and the related design. Hazard-discharging tests have observed failing baselines before passing observations count; completion derives from synced reports and a frozen Aligned four-lane review whose aggregate diff still matches, never manual graph/view edits or asserted status. (D-0022, D-0023)

## Constraints
- D-0012 and `SDD-Toolchain:NFR-04` preserve Go validation and the complete offline test contract. D-0014 preserves reviewer read-only boundaries. D-0017 preserves canonical/generated portability boundaries. D-0021 preserves user-owned binary installation; this feature does not provision binaries.
- D-0022 governs red-before-green observations and frozen four-lane review closure; D-0023 keeps work tracking in the planning root. D-0024 governs lifecycle bookkeeping boundaries. D-0003 and D-0018 forbid silent decision collisions or ledger writes without exact-text approval.
