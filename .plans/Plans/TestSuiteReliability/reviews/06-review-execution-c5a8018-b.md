...
2026-09-13: Supersedes `06-review-execution-c5a8018.md`, whose revise of `posix-containment` was refused by `sdd graph amend` as non-normative (no contract, gate, justifies or inputs change). The fix is a code-only cleanup: bind the discarded pre-reap error to `_`. It lands as an inline commit in the pre-`full-gate` cleanup batch, which re-stales `posix-containment` through the artifact digest and is re-verified and re-reviewed at that revision.
