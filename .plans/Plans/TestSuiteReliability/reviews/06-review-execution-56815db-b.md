...
### F-02 — The graph-plan exemption is directory-scoped and folds a stat failure into absence

`internal/rules/graphviews.go:77-81` answers a directory-level question but gates per-document exemptions at `headings.go:245,336` and `phasereview.go:234`; a hand-authored phase doc registered beside a graph passes with zero diagnostics (reproduced by the fixtures-gate blind-spot lane). The predicate returns false on any stat error and is an unmemoized stat from five rule sites over every artifact in the root. Raised at this identity by `Plans/TestSuiteReliability/reviews/06-review-fixtures-56815db-b.md` F-01 and carried here because propagation-validator is in this gate's closure.

### F-03 — Unused types in the evidence renderer
...
2026-09-13: Removed in the propagation-graph-callers rev 7 commit alongside F-01. This artifact supersedes `06-review-execution-56815db.md` so the validator revise can ride the same amendment.
