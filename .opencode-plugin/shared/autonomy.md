...
| Artifact writes that follow templates and evidence-gated status transitions | `sdd-plan` writing `draft`, `sdd-implement` recording completion evidence before flipping task statuses, appending a collision-free decision-ledger entry after the user decides (`shared/decision-log.md`), etc. |
