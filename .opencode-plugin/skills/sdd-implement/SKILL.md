...
6. **Between rounds.** `sdd graph status` between claims; `sdd graph path` when choosing what to unblock. **Command gates:** run the gate's command and `sdd graph sync --node <id> --command-exit <N> --command-log out.txt`. **Review nodes:** claim it, inspect `sdd graph show <id> --plan <Name> --brief`, run the four-lane review flow, then `sdd graph review --plan <Name> --node <id> --artifact <frozen review path>`. An Aligned review observation binds the reviewed scope's `contract_rev` values and observation sequences. A scope change, reviewed revision advance, or newer reviewed observation makes it stale and requires a deliberate new review. With open findings, preview and apply `sdd graph amend` through its digest fences. A revise advances `contract_rev` and clears that node's `red_seqs`; revised and extended hazard-discharging tests therefore need an actual intended RED before GREEN. Walk amended nodes through red → green → sync, then re-review.
...
- **Newer direct dependency observation:** when a direct dependency is deliberately re-verified at a higher sequence than this node's pass, this node becomes STALE. Re-run its gate and sync deliberately; file and declaration edits do not trigger freshness changes.
...
- **Changing declarations:** artifacts and inputs are review-visible declarations, never freshness keys. `set-inputs` replaces inputs on an unclaimed node (a verified node advances `contract_rev` and keeps compatible `red_seqs`; an unverified node with red observations is refused); `set-artifacts` edits the holder's write set. Neither substitutes for execution.
