...
3. Write or preserve `<target>/planning-config.json`. Include optional `title`, and `description` only when requested. Preserve every existing unrelated key. Setup never creates a second repo-root selector.
4. Create missing planning directories: `Plans/`, `Research/`, `Brainstorm/`, `Specs/`, and `Designs/`.
