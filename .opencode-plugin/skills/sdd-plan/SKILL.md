...
### 2. Gather Context (delegated to a collaboration subagent when available)

Dispatch a collaboration subagent rendering `shared/agent-prompts/researcher.md` (or perform that prompt yourself if collaboration is unavailable) and ask it to return a **structured** summary, not freeform notes:
...
- **Inserting a task into an existing phase**: give it the `<phase>.<N>a` form (`1.2a` between `1.2` and `1.3`) rather than renumbering. Ids are opaque and append-only — completion evidence, completed-task-identity roll-ups, and frozen phase reviews all cite them, so renumbering silently invalidates records that already name the old id.
- Identify: new tasks to add, existing tasks that need refinement (vague verification, missing subtasks, outdated notes), missing phases, and **tasks that should no longer exist** — work the researcher found already implemented in the target repo, superseded by a later decision, made redundant by another task or plan, or whose motivating requirement was cut. Revision is not append-only: a plan that only ever grows drifts away from the work that is actually left.
- **Retiring a task.** A non-complete task judged unnecessary is retired, not silently deleted: remove it from the `tasks[]` array and strike its body heading with a one-line reason (`~~1.3: Old Title~~ — removed: already implemented by <path>` / `superseded by D-NNNN`). Its id stays retired and is never reused, so existing cross-references cannot silently re-bind (`shared/frontmatter-schema.md` § Stable Identifiers). Completed tasks are never retired — the preservation rule below wins.
...
- **Every task is sourced.** A task exists because something demands it — a spec requirement (`FR-NN`/`NFR-NN`), an acceptance criterion (`AC-NN`), an accepted decision (`D-NNNN`), or a concrete failure it prevents. Record that demand in the task's `justifies` field (`shared/frontmatter-schema.md` § Task Fields). If the only available justification is "might need it later", "for completeness", or symmetry with a neighboring task, the task does not go in the plan. This is the counterweight to the phase/task counts above: `2-6 tasks` is a shape, not a quota — a phase with three sourced tasks is finished, and `never pad with filler phases` binds at task level too.

  Two limits keep this from cutting the wrong things. **Necessity is about sourcing, not size** — the test is "what demands this?", never "does this feel like a lot of work"; a large sourced task stays. And **correctness is never speculative**: error handling, edge cases, tests, rollback, and observability are load-bearing even when no requirement names them, and `justifies` for that work names the failure it prevents (e.g., "prevents silent data loss on partial write"). This rule targets speculative capability — the interface with one implementation, the config surface nothing varies, the extension point no requirement asks for.
...
- **`### Completion Evidence`** — required, initially exactly `Pending — not complete.`; `sdd-implement` replaces it with what actually ran before changing task status to `complete`.
...
- Run the deterministic validator first: `sdd validate --scope Plans/<PlanName> --format json` (see `sdd-validate`). Fix structural diagnostics before dispatching the reviewer — don't spend a review cycle on findings a script catches.
- Dispatch a collaboration subagent in a fresh non-inheriting context rendering `shared/agent-prompts/plan-reviewer.md` (if collaboration is unavailable, perform the review yourself following that prompt and label it **self-review**) to review the complete plan.
...
- **Mechanics:** create a scratch git worktree of the target repo (or a full copy for non-git targets) — never rehearse against the live tree. Dispatch implementation subagents (stable dispatch identifier `implement_task`) into the scratch tree to execute the plan's tasks literally, wave by wave, with the same evidence rules as `sdd-implement` (verification output pasted, STOP on plan-vs-reality mismatch).
...
- **Record decisions**: after approval, take each user-resolved open question and each Key Decision the user actually made (not ones merely drafted for them), run it through the **admission test** in `shared/decision-log.md` § Capture, and record only those that pass — sequencing and task-shaping choices are the plan's own content, while an approach or constraint that binds implementation and review beyond this plan earns an entry. Collision check before each append; a collision stops for the user. Scope entries to `Plans/<PlanName>` (or to the governing spec/design when the decision really lives there), and **cite each new entry's id inline** in the plan's Key Decisions section (e.g., "(D-0012)") — the bidirectional link is what makes supersession detection work.
...
- **Non-Goals**: What the plan deliberately does not deliver — the related specs' Non-Goals carried forward, plus boundaries decided during planning. A task that implements something listed here is a scope violation
...
- Agents: the researcher prompt (`shared/agent-prompts/researcher.md`), the plan-review prompt (`shared/agent-prompts/plan-reviewer.md`), an implementation subagent (dispatch identifier `implement_task`) (rehearsal only)
