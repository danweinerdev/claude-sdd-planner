...
3. Show the drafted entry to the user **verbatim and in full, and wait for explicit approval** — even though `sdd-decide` was an explicit ask, the exact entry text must itself be approved before it is written; an amended draft is re-shown and re-approved. Only then append the approved text unchanged to `decisions[]`, update the ledger's `updated` date, and re-read the frontmatter to confirm it parses as YAML.
